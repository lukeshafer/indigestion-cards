...
export async function getCardDesignAndOpenedInstances(args: { designId: string }) {
	const db = new Service(
		{
			cardDesigns,
			cardInstances,
	...
	const result = await db.collections
		.designAndCards(args)
		.where(
			({ openedAt }, { gt, field, escape }) =>
				`${gt(openedAt, '')} OR ${field('__edb_e__')} = ${escape('cardDesign')}`
		)
		.go({ pages: 'all' });

	return { design: result.data.cardDesigns[0], instances: result.data.cardInstances };
}
