			...
			rows={trades().map((trade) => ({
				...
				with: isLoggedInUsername(trade.senderUsername)
					? {
							...
							element: <UsernameLink username={trade.receiverUsername} />,
					  }
					: {
							value: trade.senderUsername,
							element: <UsernameLink username={trade.senderUsername} />,
					  },
				from: {
					value: trade.senderUsername,
					element: <UsernameLink username={trade.senderUsername} />,
...
function UsernameLink(props: { username: string }) {
	return (
		<a href={`${routes.USERS}/${props.username}`} class="hover:underline focus:underline">
			{props.username}
		</a>
	);
}
