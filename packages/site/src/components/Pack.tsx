...
export const Pack: Component<{
	...
	<div
		...
		<p class="absolute bottom-0 left-0 whitespace-break-spaces p-[0.5em] font-bold  leading-[0.7em]">
      {props.packNumber}
		</p>
...
export const formatPackNumber = ({
	packNumber,
	packNumberPrefix: prefix,
}: {
	packNumber?: number;
	packNumberPrefix?: string;
}): string => (prefix ? `${prefix}-${packNumber ?? 'PACK'}` : String(packNumber ?? ''));
