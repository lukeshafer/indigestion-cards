...
interface Path {
	...
	//'site-config': typeof import('../pages/data/site-config');
}
...
// @ts-ignore -- doesn't need to be used
type _CHECKS = [
	Expect<Path[keyof Path] extends PathType ? true : false>,
	Expect<ParamPath[keyof ParamPath]['output'] extends PathType ? true : false>,
];

type ExpectedApiGetter = (ctx: APIContext) => Promise<TypedResponse<any>>;
type PathType = {
